...
const Box=({position,color})=>{
  ...
  useFrame(()=>(mesh.current.rotation.z = mesh.current.rotation.x = mesh.current.rotation.y +=0.015));
...
const Sphere=({position, color})=>{
  ...
  return (
       <a.mesh onClick={()=>setExpand(!expand)} scale={props.scale} position={position} ref={mesh}>
         <sphereBufferGeometry attach="geometry" args={[2,10,15]} />
         <meshStandardMaterial attach="material" color={color} />
       </a.mesh>     
  );
}
...
const Cylinder=({position,color})=>{
  ...
  return (
    <>
       <a.mesh onClick={()=>setExpand(!expand)} scale={props.scale} position={position} ref={mesh}>
         <cylinderBufferGeometry attach="geometry" args={[1,1,4,10]} />
         <meshStandardMaterial attach="material" color={color} />
       </a.mesh>   
    </>  
  );
    ...
    <>
     ...
     camera={{position:[-5,2,10], few:40}} >
      ...
      <group>
       ...
       <Sphere position={[4,1,1]} color="#ff87ab" />
     </Canvas>
    </>
  )
